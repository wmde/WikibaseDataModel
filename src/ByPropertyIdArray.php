...
class ByPropertyIdArray {
	 ...
	 */
	public function __construct( $propertyIdProviders = array() ) {
	 ...
	 *
	 * @param int $index
	 * @param bool $checkForEquality
	 *
	 * @throws InvalidArgumentException
	 * @throws OutOfBoundsException
	 */
	private function assertValidIndex( $index, $checkForEquality = true ) {
		if ( !is_int( $index ) ) {
		...
		if ( $index === count( $this->flatArray ) && $checkForEquality ) {
			throw new OutOfBoundsException( 'The index exceeds the array dimensions.' );
		}
	}
